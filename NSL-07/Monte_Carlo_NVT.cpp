...
void PrintIstantValues(void){
    // Print instantaneous values of U/N and P
    ofstream inst_Epot_P;
    inst_Epot_P.open("results/instant_epot_pres.dat",ios::app);

    double stima_inst_pot = walker[iv]/(double)npart + vtail;
    double stima_inst_pres = rho * temp + (walker[iw] + ptail * (double)npart) / vol;

    inst_Epot_P << stima_inst_pot << " " << stima_inst_pres << endl;
    inst_Epot_P.close();
}
...
//g(r)
    for(int i=0; i< nbins; i++){
        ...
        bin = igofr + (i+1);
        r = i*bin_size;
        stima_gofr = 1./(rho*(double)npart*4./3.*M_PI*(pow(r+bin_size, 3)-pow(r, 3)) )*blk_av[bin]/blk_norm;

        glob_av[bin] += stima_gofr;
        glob_av2[bin] += stima_gofr*stima_gofr;
        err_gofr=Error(glob_av[bin],glob_av2[bin],iblk);

        // g(r) average in each block
        Gofr << iblk << " " << r + 0.5*bin_size << " " << stima_gofr << endl;
        //g(r) averaged over all blocks
        if(iblk==nblk) Gave << iblk << " " << r + 0.5*bin_size << " " << glob_av[bin]/(double)iblk << " " << err_gofr << endl;
    }
